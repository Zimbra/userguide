...
=== 開封確認を要求
送信したメールに対して、宛先が開封した際に開封確認を送信させるように設定することが可能です。

メールを作成する際、画面の右上にある[…]メニュー (image:graphics/ellipsis-h.svg[width=20px]) をクリックし、「開封確認を要求」のオプションを選択します。

なお、この設定はあくまでも「要求」であるため、宛先側には開封確認の送信を許可しない権利があります。詳細については <<settings-viewEmail.adoc#_開封確認>> を参照してください。
